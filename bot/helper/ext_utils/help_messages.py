...
message. ``-bh`` is a shortcut for <code>-up bh</code>.

Works with <code>-ad</code> too (e.g. <code>/m magnet -ad -bh</code>):
AllDebrid resolves the magnet/torrent, then the files are uploaded to
BuzzHeavier. When replying to a Telegram file, use <code>/m -bh</code>
on its own (the link must come before flags, so a bare reply works).

Optionally set <code>BUZZHEAVIER_ACCOUNT_ID</code> in the bot
